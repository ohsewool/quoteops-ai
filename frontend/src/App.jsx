...
const INTERNAL_CODE_LABELS = {
  ...
  standard_margin: "기준 마진율",
  standard_margin_custom: "기본 마진 전략",
  conservative_bulk: "보수형 대량 주문",
  price_above_unit_cost: "원가 초과 여부",
  margin_meets_minimum: "최소 마진 기준",
  minimum_margin_threshold_met: "최소 마진 기준 충족",
  competitor_reference_below_average: "경쟁사 평균 하회 여부",
  competitor_reference_above_average: "경쟁사 평균 초과 여부",
...
const VISIBLE_TEXT_LABELS = {
  "QuoteOps AI End-to-End Demo": "QuoteOps AI 데모 흐름",
  "DEMO-BANNER-001": "데모 배너",
  "Review demo data status": "데모 데이터 상태 확인",
  "Seed deterministic demo data": "샘플 데이터 준비",
  "Generate quote preview": "견적 미리보기 생성",
  "Generate candidate prices": "가격안 생성",
  "Validate proposed price": "제안 가격 평가",
  "Submit human approval request": "승인 요청 제출",
  "Compare scenarios": "시나리오 비교",
  "Generate deterministic HTML report": "리포트 생성",
  "Deterministic candidate generated from unit cost and margin rate.": "원가와 마진 기준으로 계산한 가격안입니다.",
  "No AI-generated price was used.": "AI가 가격을 생성하지 않았습니다.",
  "No AI approval decision was used.": "AI가 승인 결정을 하지 않았습니다.",
  "No price table was automatically activated.": "가격표가 자동 적용되지 않았습니다.",
  "No websites were scraped and no emails were sent.": "외부 웹사이트를 수집하거나 이메일을 보내지 않았습니다.",
  "Demo tools are for local development and presentations.": "데모 도구는 포트폴리오 시연용입니다.",
  "Demo competitor prices are manually entered references, not real market prices.": "경쟁사 가격은 수동 입력한 참고값이며 실제 시장 가격이 아닙니다.",
  "Demo tools do not approve, reject, or activate production prices.": "데모 도구는 실제 가격을 승인, 반려, 적용하지 않습니다.",
  "Demo tools do not scrape websites, call external AI, or send emails.": "데모 도구는 외부 웹사이트, 외부 AI, 이메일 발송을 사용하지 않습니다.",
  "Do not claim demo competitor prices are real market data.": "데모 경쟁사 가격을 실제 시장 데이터라고 설명하지 마세요.",
  "Do not claim AI generated the numeric prices.": "숫자 가격을 AI가 생성했다고 설명하지 마세요.",
  "Do not claim demo approvals are production approval decisions.": "데모 승인을 실제 운영 승인이라고 설명하지 마세요.",
  "Do not claim demo reset deletes arbitrary production data.": "데모 초기화가 실제 운영 데이터를 삭제한다고 설명하지 마세요.",
}
...
function displayNote(note) {
  ...
  if (VISIBLE_TEXT_LABELS[text]) return VISIBLE_TEXT_LABELS[text]
  if (lowered.includes("this quote needs human review")) {
    return "검토가 필요합니다. 제안 가격은 사용할 수 있지만 최소 마진 기준을 다시 확인해야 합니다."
  }
  if (lowered.includes("the proposed unit price is higher") || lowered.includes("candidate price is above unit cost")) {
    return "제안 단가가 계산된 원가보다 높습니다."
  }
  if (lowered.includes("the estimated margin is approximately")) {
    const margin = text.match(/approximately\s+([0-9.]+%)/i)?.[1]
    return `예상 마진은 약 ${margin || "25.0%"}입니다.`
  }
  if (lowered.includes("estimated margin is below the minimum margin threshold")) {
    return "예상 마진이 최소 기준보다 낮습니다."
  }
  if (lowered.includes("more than 20% below the competitor average")) {
    return "제안 가격이 경쟁사 평균보다 20% 이상 낮습니다."
  }
  if (lowered.includes("not far above the competitor average")) {
    return "제안 가격이 경쟁사 평균보다 과도하게 높지 않습니다."
  }
  if (lowered.includes("this explanation is written for")) {
    return "관리자가 검토할 수 있도록 작성한 설명입니다."
  }
  if (lowered.includes("no price was generated by ai") || lowered.includes("no ai-generated") || lowered.includes("ai-generated")) {
    return "AI가 가격을 생성하지 않았습니다."
  }
  if (lowered.includes("no approval decision was made by ai")) {
    return "AI가 승인 또는 반려 결정을 하지 않았습니다."
  }
  if (lowered.includes("a human reviewer must still approve")) {
    return "사람이 최종 승인하거나 반려해야 합니다."
  }
  if (lowered.includes("comparison is deterministic") || lowered.includes("deterministic")) {
    return "기준에 따라 비교한 결과입니다."
  }
  if (lowered.includes("this comparison does not approve")) {
    return "이 비교 결과만으로 가격이 승인되거나 적용되지 않습니다."
  }
...
function displaySource(value) {
  if (!value) return "-"
  if (value === "deterministic_template") return "기준 설명 템플릿"
  return displayCode(value)
}
    ...
    <main className="app-shell min-h-screen bg-slate-100 text-slate-950">
      <div className="app-main page-container mx-auto max-w-7xl px-5 py-6">
        ...
        <section className="section card-grid mb-5 grid gap-4 lg:grid-cols-[1fr_1.4fr]">
          ...
          {showSection("admin-system") && currentUser && dashboardSummary && (
            <Panel title="운영 요약">
          ...
          {showSection("admin-system") && currentUser && dashboardInsights && (
            <Panel title="분석 인사이트">
              <div className="flex flex-wrap gap-2">
            ...
            <section className="support-page">
              ...
              {demoScenario && (
                <div className="rounded-md border border-slate-200 bg-slate-50 p-3">
                  <div className="mb-2 flex flex-wrap gap-2">
                    <Badge>{displayNote(demoScenario.scenario_name)}</Badge>
                    <Badge>{displayNote(demoScenario.demo_product_sku)}</Badge>
        ...
        <section className="workflow-page section">
          ...
          <div className="grid gap-5">
              ...
              <div className="grid gap-3 md:grid-cols-3">
                {results.candidates?.candidates?.map((candidate) => (
                  <div className="rounded-md border border-slate-200 bg-slate-50 p-4" key={candidate.strategy}>
                    ...
                    <Notes notes={displayNotes(candidate.notes)} />
                  </div>
                ))}
              </div>
              ...
              {results.validation && (
                <div className="space-y-3">
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Badge>마진율: {results.validation.estimated_margin_rate}</Badge>
                  </div>
                ...
                {activeSimulation && (
                  <div className="space-y-3">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Badge>시나리오: {activeSimulation.scenario_count}</Badge>
                      <Badge>단위 원가: {formatMoney(activeSimulation.unit_cost)}</Badge>
                    </div>
                    <div className="table-wrap">
                      <table>
                        <thead>
                      ...
                      <tbody>
                        {priceTableComparison.changes.map((change) => (
                          <tr key={`${change.product_id}-${change.change_type}`}>
                            ...
                            <td>{displayStatus(change.change_type)}</td>
            ...
            <Panel title="안전 설명">
              ...
              {results.explanation && (
                <div className="space-y-3">
                  <p className="rounded-md bg-slate-950 p-4 text-white">{displayNote(results.explanation.explanation_summary)}</p>
                  <Notes notes={displayNotes(results.explanation.explanation_bullets)} />
                  <Notes title="결정 기준" notes={displayNotes(results.explanation.decision_boundaries)} />
                  <details className="advanced-details">
                    <summary>기술 정보</summary>
                    <Badge>출처: {displaySource(results.explanation.explanation_source)}</Badge>
                  </details>
                </div>
              )}
            </Panel>
  ...
  return (
    <div>
      ...
      <ul className="list-disc space-y-1 pl-5 text-sm text-slate-600">
        {notes.map((note) => {
          const display = displayNote(note)
          return <li key={String(note)}>{display}</li>
        })}
      </ul>
    </div>
  )
