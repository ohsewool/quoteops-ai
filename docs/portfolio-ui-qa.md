...
PR-45에서는 공개 랜딩 문구를 `견적부터 승인까지 한 흐름으로` 중심으로 정리하고, 안전 문구를 `최종 가격은 승인 후 확정`으로 더 명확하게 바꿨다. 내부 화면의 표시용 필드명과 상태값은 한국어 우선으로 보여주되 backend/API 필드명과 payload key는 그대로 유지한다.

기술 상태와 OpenAPI 같은 내부 점검 정보는 운영 화면에만 둔다. 데모 흐름은 여전히 공개 랜딩의 `데모 체험하기`에서 시작한다.
